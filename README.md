...
To generate 12 groups with randomly selected, well-spread K values in the
range 50-100, run. The selector uses MiniBatchKMeans to keep the 300K-row run
practical:
...
When running on a high-core server, limit BLAS/OpenMP threads before starting
the selector, as done by `scripts/kmeans_select.sh`.

Outputs are written to `data/sft/kmeans/` as
`kmeans_01.jsonl` through `kmeans_12.jsonl`, plus a manifest. Each line keeps
`candidate_index`, `sample_id`, `source`, and `messages`; no new DOC file or
token-count calculation is performed. The selected K values are recorded in
the manifest and are reproducible from `--k-range`, `--groups`, and `--seed`.

## SCS Score for Selected Datasets

Calculate SCS for one selected JSONL file or all JSONL files in a directory:

```bash
python -m src.scs_score \
    --input data/sft/random \
    --candidate-doc data/candidate/v1/candidate_doc.jsonl \
    --embedding-npz output/feature/candidate_embeddings_llama.npz \
    --logprob-npz output/feature/candidate_logprob_qwen.npz \
    --tokenizer FacebookAI/xlm-roberta-large \
    --output-dir output/scs/random \
    --overwrite
```

The scorer uses each selected record's `candidate_index` to read features and
DOC text from the frozen candidate pool. It runs semantic clustering for each
selected dataset and writes `<dataset>_SCS.json`, including `avg_token_num` in
each class. No selected-subset DOC file is created.
