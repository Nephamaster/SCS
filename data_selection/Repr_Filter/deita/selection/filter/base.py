...
class IterativeFilter(object):
    ...
    def filter(self, df):
        ...
        print(f"Data number before filtering: #{len(df)}")
        ...
        for i in tqdm(range(1, embeddings.shape[0], self.batch_size), total = embeddings.shape[0] // self.batch_size):
            ...
            # print('distance_existed: ', distance_existed)
            ...
            # print('distance_cur: ', distance_cur)
            distance_cur_bool = torch.any(distance_cur > self.threshold, dim = 1)
            # print('distance_cur_bool: ', distance_cur_bool.tolist())
            distance_bool = distance_existed_bool | distance_cur_bool
            # print('distance_bool: ', distance_bool.tolist())
            # cnt += 1
            # if cnt ==5:break
            ...
            if len(filtered_indices) - start_cnt > 1000:
                ...
                print("Now data number: #{}".format(len(filtered_indices)))
        ...
        print(f"Data number after filtering: #{len(df_filtered)}")
        if self.data_size > -1:
